	...
	for workspace in workspaces:
		...
		if ticks == 1000:
			r1 = workspace.report('count turtles with [shape = "circle"]')
                        ...
                        print(str(200 - len(workspaces))+" "+str(r1)+" "+str(r2)+" "+str(r3)+" "+str(r4)+" "+str(ticks))
			workspaces.remove(workspace)
stopTime = int(round(time.time() * 1000))
totalTime = stopTime - startTime
with open("Times_Comparison_Ethnocentrism_2.csv", "a+") as myfile:
        myfile.write('Ethnocentrism,200,PyNetLogo,' + str(totalTime))
print(totalTime)
